...
function tampilPetunjukKategori(){
    ...
    // Update placeholder baris-baris item yang sudah terlanjur ada di form
    let placeholderJumlah = kategori === "snack" ? "Bal" : "Jumlah";
    let placeholderHarga = kategori === "snack" ? "Harga/Bal" : "Harga Satuan";

    document.querySelectorAll(".baris-item").forEach(baris=>{
        baris.querySelector(".jumlahItem").placeholder = placeholderJumlah;
        baris.querySelector(".hargaItem").placeholder = placeholderHarga;
    });
...
function tambahBarisItem(){
    ...
    let kategori = document.getElementById("kategoriBelanja")
        ? document.getElementById("kategoriBelanja").value
        : "snack";
    ...
    <div class="baris-item" id="${idBaris}">
        ...
        <input class="jumlahItem" type="number" step="0.1" placeholder="${placeholderJumlah}">
        <input class="hargaItem" type="number" placeholder="${placeholderHarga}">
...
function tampilBelanja(data = belanja){
    ...
    Object.keys(kelompok).forEach((key)=>{
        ...
        let htmlKartu = grup.items.map((b)=>{
            ...
            let satuan = (!b.kategori || b.kategori === "snack") ? "bal" : "";

            let htmlItems = items.map(it => `
                <div class="item-produk">
                ${it.kode ? it.kode + " - " : ""}${it.nama || ""} :
                ${it.jumlahBal || 0}${satuan ? " " + satuan : ""} &times;
            ...
            <div class="card">
                ...
                <h3>${formatTanggalID(b.tanggal)}</h3>
...
// CATAT/PERBARUI/HAPUS ENTRI KAS OTOMATIS TERKAIT SATU TRANSAKSI BELANJA
// LANGSUNG DIEKSEKUSI KE FIREBASE SAAT ITU JUGA (tidak menunggu menu Kas dibuka)
async function catatKasOtomatis(sumberModul, sumberId, kasKey, jenis, nominal, catatan, tanggalTransaksi){
    ...
    for(const t of lama){
        try {
            await deleteDoc(doc(db, "kas", t.id));
        } catch (err) {
            let antrianHapusKas = JSON.parse(localStorage.getItem("kasHapusAizzay")) || [];
            antrianHapusKas.push(t.id);
            localStorage.setItem("kasHapusAizzay", JSON.stringify(antrianHapusKas));
            console.log("Gagal hapus kas " + t.id + " langsung (mungkin offline), dimasukkan antrian:", err.message);
        }
    }
    ...
    if(nominal > 0){
        ...
        let entriBaru = {
            ...
            tanggal: tanggalTransaksi || new Date().toISOString().slice(0, 10),
            kas: kasKey,
            jenis: jenis,
            nominal: nominal,
            catatan: catatan,
        ...
        try {
            await setDoc(doc(db, "kas", entriBaru.id), entriBaru);
            let kasTerkini = JSON.parse(localStorage.getItem("kasAizzay")) || [];
            let idx = kasTerkini.findIndex(k => k.id === entriBaru.id);
            if(idx >= 0){
                kasTerkini[idx].sinkron = true;
                localStorage.setItem("kasAizzay", JSON.stringify(kasTerkini));
            }
        } catch (err) {
            console.log("Gagal kirim kas " + entriBaru.id + " langsung (mungkin offline), akan disinkron nanti dari menu Kas:", err.message);
        }
...
function simpanKulakan(){
    ...
    let labelJenisBelanja =
        kategori === "snack" ? "Kulakan" :
        kategori === "plastik" ? "Belanja Plastik" :
        "Belanja Lain-lain";
    ...
    catatKasOtomatis(
        ...
        "keluar",
        data.jumlahDibayar,
        labelJenisBelanja + " " + data.supplier + " (" + formatTanggalID(data.tanggal) + ")",
        data.tanggal
    );
...
function hapusBelanja(index){
    ...
    let konfirmasi = confirm(
        "Yakin hapus kulakan tanggal " + formatTanggalID(b.tanggal) + " (" + b.supplier + ")?"
    );
...
function lunasBelanja(index){
    ...
    let labelJenisPelunasan =
        belanja[index].kategori === "snack" || !belanja[index].kategori ? "Pelunasan kulakan" :
        belanja[index].kategori === "plastik" ? "Pelunasan belanja plastik" :
        "Pelunasan belanja lain-lain";
    ...
    catatKasOtomatis(
        ...
        labelJenisPelunasan + " " + belanja[index].supplier + " (" + formatTanggalID(belanja[index].tanggal) + ")"
    );
