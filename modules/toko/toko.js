    ...
    urut.forEach((t)=>{
        ...
        if(t.status === "selesai"){
            ...
            <div class="card">
                ...
                <h3>${formatTanggalID(t.tanggalKirim)} &rarr; ${formatTanggalID(t.tanggalSettle)}</h3>
            ...
            <div class="card">
                ...
                <h3>${formatTanggalID(t.tanggalKirim)}</h3>
...
function simpanKirim(){
    ...
    if(sudahAda){
        let lanjut = confirm(
            "Sudah ada titipan untuk \"" + toko + "\" tanggal " + formatTanggalID(tanggalKirim) + ".\n" +
...
function bukaSettle(index){
    ...
    document.getElementById("infoSettle").textContent =
        t.toko + " - dikirim " + formatTanggalID(t.tanggalKirim);
...
async function catatKasOtomatis(sumberModul, sumberId, kasKey, jenis, nominal, catatan, tanggalTransaksi){
    ...
    // HAPUS ENTRI LAMA LANGSUNG DI FIREBASE; KALAU GAGAL (MISAL OFFLINE), MASUKKAN ANTRIAN SEPERTI BIASA
    for(const t of lama){
        try {
    ...
    if(nominal > 0){
        ...
        // KIRIM ENTRI BARU LANGSUNG KE FIREBASE JUGA; KALAU GAGAL, BIARKAN sinkron:false SEPERTI BIASA (nanti disinkron dari menu Kas)
        try {
...
function simpanSettle(){
    ...
    catatKasOtomatis(
        ...
        "masuk",
        t.jumlahDibayar,
        "Setoran toko " + (t.toko || "") + " (" + formatTanggalID(t.tanggalSettle) + ")",
        t.tanggalSettle
    );
...
function cetakNota(index){
    ...
    if(t.status === "selesai"){
        ...
        <body>
            ...
            <h1>AIZZAY SNACK</h1>
            <div class="sub">
                Nota Pengambilan : ${formatTanggalID(t.tanggalSettle)}<br>
                (Titipan dikirim ${formatTanggalID(t.tanggalKirim)})<br>
                Toko : ${t.toko || ""}
            </div>
        ...
        <body>
            ...
            <h1>AIZZAY SNACK</h1>
            <div class="sub">
                Nota Titipan : ${formatTanggalID(t.tanggalKirim)}<br>
                Toko : ${t.toko || ""}
            </div>
...
function lunasTitipan(index){
    ...
    catatKasOtomatis(
        ...
        "Pelunasan setoran toko " + (titipan[index].toko || "") + " (" + formatTanggalID(titipan[index].tanggalSettle) + ")"
    );
...
function hapusTitipan(index){
    ...
    let konfirmasi = confirm(
        "Yakin hapus titipan " + t.toko + " tanggal " + formatTanggalID(t.tanggalKirim) + "?"
    );
