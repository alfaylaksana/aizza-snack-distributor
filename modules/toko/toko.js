...
function cetakNota(index){
    ...
    let baris = items.map(it => {
        ...
        <tr>
            <td>${it.jumlahKirim || 0}</td>
            <td>${it.nama || ""}</td>
            <td class="kanan">${Number(it.hargaToko || 0).toLocaleString()}</td>
            <td class="kanan">${subtotal.toLocaleString()}</td>
        </tr>
        `;
    }).join("");
    ...
    <style>
        ...
        .total-row td{ font-weight:bold; border-top:2px solid #c96b24; border-bottom:none; }
    </style>
    </head>
    <body>
        ...
        <tr class="total-row">
            ...
            <td class="kanan">Rp ${total.toLocaleString()}</td>
        </tr>
        </table>
