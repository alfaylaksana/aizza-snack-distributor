    ...
    urut.forEach((t)=>{
        ...
        if(t.status === "selesai"){
            ...
            let htmlItems = `
                ...
                <tr>
                    <td>${it.nama || ""}</td>
                    <td class="kanan">${it.jumlahKirim || 0}</td>
                    <td class="kanan">${it.jumlahRetur || 0}</td>
                    <td class="kanan">${it.jumlahTerjual || 0}</td>
...
function cetakNota(index){
    ...
    if(t.status === "selesai"){
        ...
        <html>
        <head>
        <title>Nota Selesai - ${t.toko}</title>
        <meta charset="UTF-8">
        <style>
        ...
        <body>
            ...
            <h1>AIZZAY SNACK</h1>
            <div class="sub">
                Nota Selesai : ${t.tanggalSettle || ""}<br>
...
function kirimWA(index){
    ...
    if(t.status === "selesai"){
        ...
        teks += "Nota Selesai: " + (t.tanggalSettle || "") + "\n";
        teks += "(Titipan dikirim " + (t.tanggalKirim || "") + ")\n";
        teks += "Toko: " + (t.toko || "") + "\n\n";

        teks += "```\n";
        teks += rataKiri("Nama", 12) + rataKanan("Krm", 4) + rataKanan("Ret", 4) + rataKanan("Lk", 4) + "   " + rataKanan("Subtotal", 9) + "\n";
        teks += "-".repeat(36) + "\n";

        items.forEach(it=>{
            teks += rataKiri(it.nama || "", 12) +
                rataKanan(it.jumlahKirim || 0, 4) +
                rataKanan(it.jumlahRetur || 0, 4) +
                rataKanan(it.jumlahTerjual || 0, 4) + "   " +
                rataKanan(Number(it.subtotal || 0).toLocaleString("id-ID"), 9) + "\n";
        });

        teks += "-".repeat(36) + "\n";
        teks += rataKiri("TOTAL", 24) + "   " + rataKanan(total.toLocaleString("id-ID"), 9) + "\n";
        teks += "```\n";
        ...
        teks += "Nota Titipan: " + (t.tanggalKirim || "") + "\n";
        teks += "Toko: " + (t.toko || "") + "\n\n";
