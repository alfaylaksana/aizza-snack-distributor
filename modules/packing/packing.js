...
// TAMPIL/SEMBUNYIKAN REKAP UPAH PER PEKERJA (BULAN BERJALAN)
function toggleRekapUpah(){

    let box = document.getElementById("rekapUpahPekerja");
    ...
    let rekap = {}; // { namaPekerja: { totalUpah, totalBungkus, jumlahSesi } }
    let grandTotal = 0;
    ...
    packing.forEach(p=>{
        ...
        let nama = p.pekerja || "(Belum diisi)";

        if(!rekap[nama]){
            rekap[nama] = { totalUpah: 0, totalBungkus: 0, jumlahSesi: 0 };
        }

        rekap[nama].totalUpah += Number(p.upah || 0);
        rekap[nama].totalBungkus += Number(p.hasilBungkus || 0);
        rekap[nama].jumlahSesi += 1;

        grandTotal += Number(p.upah || 0);

    });

    let namaList = Object.keys(rekap);

    if(namaList.length === 0){
        box.innerHTML = "<p style='font-size:12px;color:#888;'>Belum ada packing bulan ini.</p>";
        box.style.display = "block";
        return;
    }

    let baris = namaList.map(nama=>{
        let r = rekap[nama];
        return `
        <div class="baris-transaksi">
            <div>
                <b>${nama}</b>
                <div class="baris-catatan">${r.jumlahSesi}x packing, ${r.totalBungkus} bks</div>
            </div>
            <span class="hasil-bungkus">Rp ${r.totalUpah.toLocaleString("id-ID")}</span>
        </div>
        `;
    }).join("");

    box.innerHTML = baris + `
        <div class="baris-transaksi" style="border-top:2px solid #c96b24;margin-top:4px;padding-top:8px;">
            <b>TOTAL</b>
...
// HITUNG TOTAL UPAH SATU PEKERJA PADA SATU TANGGAL (GABUNGAN SEMUA SESI PACKING HARI ITU)
function hitungTotalUpahHarian(pekerja, tanggal){

    return packing
        .filter(p => (p.pekerja || "(Belum diisi)") === pekerja && p.tanggal === tanggal)
        .reduce((total, p)=> total + Number(p.upah || 0), 0);

}


// SINKRONKAN 1 ENTRI KAS (AKUMULASI) UNTUK SATU PEKERJA PADA SATU TANGGAL
function sinkronKasUpahHarian(pekerja, tanggal){

    let namaPekerja = pekerja || "(Belum diisi)";
    let totalUpah = hitungTotalUpahHarian(namaPekerja, tanggal);

    // sumberId dibuat dari kombinasi tanggal+pekerja, jadi semua sesi packing
    // pekerja itu di hari itu "menumpuk" jadi 1 entri Kas yang sama, bukan dobel
    let sumberId = tanggal + "_" + namaPekerja;
    ...
    catatKasOtomatis(
        ...
        "keluar",
        totalUpah,
        "Upah packing " + namaPekerja + " (" + tanggal + ")"
    );
...
function simpanPacking(){
    ...
    let pekerja = document.getElementById("pekerjaPacking").value.trim();
    ...
    sinkronKasUpahHarian(pekerja, tanggal);
...
function hapusPacking(id){
    ...
    // Hitung ulang total upah hari itu untuk pekerja ini (entri Kas otomatis ikut disesuaikan/dihapus)
    sinkronKasUpahHarian(p.pekerja, p.tanggal);
        ...
        let htmlBaris = grup.items.map((p)=>{
            ...
            <div class="baris-transaksi">
                <div>
                    ...
                    ${p.pekerja ? '<span class="label-kas">' + p.pekerja + '</span>' : ""}
...
window.toggleRekapUpah = toggleRekapUpah;
...
prosesAntrianHapus().then(ambilPackingFirebase).then(sinkronKeFirebase);
