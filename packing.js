...
async function catatKasOtomatis(sumberModul, sumberId, kasKey, jenis, nominal, catatan){
...
function catatAmbilPacking(){
    ...
    if(sesiBerjalan){
        ...
        alert(
            ...
            daftarLama + " (diambil " + formatTanggalID(sesiBerjalan.tanggalAmbil) + ")\n\n" +
...
function selesaikanPacking(id){
    ...
    let inputTgl = document.getElementById("tglSelesai_" + id);
    let tanggalSelesai = inputTgl ? inputTgl.value : "";
    ...
    rec.status = "selesai";
    rec.tanggalSelesai = tanggalSelesai;
...
function tampilSedangProses(){
    ...
    area.innerHTML = berjalan.map(p=>{
        ...
        <div class="baris-proses">
            ...
            <div class="baris-catatan">Diambil: ${formatTanggalID(p.tanggalAmbil)}${p.catatan ? " — " + p.catatan : ""}</div>
            <label style="margin-top:6px;">Tanggal Selesai</label>
            <input type="date" class="tglSelesaiInput" id="tglSelesai_${p.id}" value="${new Date().toISOString().slice(0, 10)}">
        ...
        let htmlRiwayat = riwayat.length === 0
            ...
            : riwayat.map(b=>`
                <div class="baris-transaksi">
                    <div>${formatTanggalID(b.tanggal)} — Rp ${Number(b.nominal).toLocaleString("id-ID")}</div>
...
function hapusBayarUpah(id){
    ...
    if(!confirm("Yakin hapus pembayaran Rp " + Number(b.nominal).toLocaleString("id-ID") + " ke " + b.pekerja + " (" + formatTanggalID(b.tanggal) + ")?\n\nEntri Kas yang terbentuk otomatis dari pembayaran ini juga akan ikut terhapus.")){
        return;
    }
...
function debugStokBal(kode){
    ...
    belanjaMentah.forEach(b=>{
        ambilItemsBelanja(b).forEach(it=>{
            if(it.kode === kode){
                cocok.push((formatTanggalID(b.tanggal) || "?") + " : " + it.jumlahBal + " bal");
            }
        });
    });
    ...
    let rincianPacking = recordPacking.length > 0
        ? recordPacking.map(p =>{
            ...
            return "id:" + p.id + "\nstatus: [" + p.status + "]\nbalDipakai(" + kode + "): " + (it ? it.balDipakai : 0) +
                "\npekerja: " + p.pekerja + "\nambil: " + formatTanggalID(p.tanggalAmbil) + " / selesai: " + formatTanggalID(p.tanggalSelesai);
          }).join("\n---\n")
            ...
            <div class="baris-transaksi">
                <div>
                    ...
                    <div class="baris-catatan">
                        Ambil: ${formatTanggalID(p.tanggalAmbil)} → Selesai: ${formatTanggalID(p.tanggalSelesai)}<br>
